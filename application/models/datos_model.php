...
public function get_where_limit($tabla=null,$campos=null,$ordenar=null,$orden=null,$filtro=null,$limit=null){
    ...
    public function get_val_ventas(){
        $sSQL="";
        $sSQL.=" SELECT SUM(total) AS ventas, count(row_id) as cont";
        $sSQL.=" FROM  pagos";
        return $this->db->query($sSQL);
    }
    public function get_estadisticas(){
        $sSQLs="";
        $sSQLs.=" SELECT *,MONTH(fecha_origen) as mes  FROM  pagos ORDER BY fecha_origen";
        return $this->db->query($sSQLs);
    }
    public function get_estads_mes(){
        $sSQLs=" SELECT MONTH(fecha_origen) as mes, sum(total) as suma";
        $sSQLs.=" FROM  pagos ";
        $sSQLs.=" WHERE fecha_origen BETWEEN DATE_SUB(CURDATE(), INTERVAL 3 MONTH) AND NOW()";
        $sSQLs.=" GROUP BY mes ORDER BY mes";
        return $this->db->query($sSQLs);
    }
